...
With tutorials enabled, stage 1 reuses the title screen's keyboard sketch directly above DEX. It follows his screen position until the camera begins scrolling, then fades out over 0.8 real seconds. Jumping in place or pressing toward the start boundary does not dismiss it. The A/Z section returns in the HUD for 6 seconds on the first 3 qualifying pickups combined per new run: stored candy, a newly equipped manual pump, or a pen stored in that manual pump. Automatic equipment, immediately consumed pens and candy canes do not trigger it. The count carries across levels and lives; a new run resets it. These passive sketches never slow the game or depend on BG. Reading a full tip temporarily preserves their remaining display time. Switching tutorials off hides them immediately.

First-use tips close automatically after 12 real seconds. A visible countdown bar shows the remaining time: the first 10 seconds run at 6% speed, then world movement, hazards, remaining time and physiology smoothly return to full speed over 2 seconds. No Enter confirmation is required. Cache explanations are omitted so players can discover them. The tutorial toggle persists through localStorage on the device. Rejected storage access falls back to the session setting. Encounter tips are remembered within the current page session; disabling and re-enabling tutorials does not replay already discovered encounters. Pickup tips describe fixed controls and inventory, never BG-triggered dosing instructions.
